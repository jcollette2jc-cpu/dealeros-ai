...
function validate(row: ImportRow, rowNumber: number) {
  ...
  return {
    ...
    color: row.color?.trim() || null, status, notes: row.notes?.trim() || null,
    archivedAt: null,
  };
}

export async function importVehicles(rows: ImportRow[]): Promise<ImportResult> {
  if (!Array.isArray(rows) || rows.length === 0) return { created: 0, updated: 0, skipped: 0, errors: [{ row: 0, message: "No rows were provided." }] };
  if (rows.length > 2000) return { created: 0, updated: 0, skipped: rows.length, errors: [{ row: 0, message: "Imports are limited to 2,000 vehicles at a time." }] };

  const result: ImportResult = { created: 0, updated: 0, skipped: 0, errors: [] };
  for (let index = 0; index < rows.length; index += 1) {
    try {
      const data = validate(rows[index], index + 2);
      const existing = await prisma.vehicle.findFirst({ where: { OR: [{ vin: data.vin }, { stockNumber: data.stockNumber }] } });
      if (existing) { await prisma.vehicle.update({ where: { id: existing.id }, data }); result.updated += 1; }
      else { await prisma.vehicle.create({ data }); result.created += 1; }
    } catch (error) {
      result.skipped += 1;
