...
import { archiveLead, completeFollowUp, createLead, scheduleFollowUp, updateLeadStatus } from "./actions";
      ...
      <div className="mx-auto max-w-[1600px]">
        <header className="mb-8 flex flex-col gap-4 border-b border-slate-800 pb-6 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-400">Create leads, control follow-ups, track appointments, and protect response speed.</p>
          </div>
          <a href="#new-lead" className="rounded-lg bg-blue-600 px-5 py-3 text-center text-sm font-semibold hover:bg-blue-500">Add Lead</a>
        </header>
        ...
        <section id="new-lead" className="mt-8 rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <form action={createLead} className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <select name="vehicleId" className={inputClass} defaultValue="">
              <option value="">Open inquiry</option>
              {vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.year} {vehicle.make} {vehicle.model} · #{vehicle.stockNumber}</option>)}
            </select>
            ...
            <button className="rounded-lg bg-blue-600 px-5 py-3 text-sm font-semibold hover:bg-blue-500">Save Lead</button>
          </form>
        </section>
              ...
              <article key={lead.id} className={`rounded-2xl border bg-slate-900 p-5 ${isOverdue ? "border-yellow-500/50" : "border-slate-800"}`}>
                <div className="grid gap-5 xl:grid-cols-[1.3fr_1fr_1.4fr] xl:items-start">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="mt-2 text-slate-400">Appointment: {lead.appointmentAt?.toLocaleString() ?? "None"}</p>
                    <p className="mt-2 text-slate-500">Last contact: {lead.lastContactAt?.toLocaleString() ?? "None"}</p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <form action={updateLeadStatus.bind(null, lead.id)} className="flex gap-2 sm:col-span-2">
                      <Select name="status" options={["New", "Contacted", "Appointment", "Working", "Sold", "Lost"]} defaultValue={lead.status} />
                      <button className="rounded-lg bg-slate-700 px-4 py-2 text-sm font-semibold hover:bg-slate-600">Update</button>
                    </form>
                    <form action={scheduleFollowUp.bind(null, lead.id)} className="flex gap-2 sm:col-span-2">
                      <Input name="nextFollowUpAt" type="datetime-local" required />
                      <button className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold hover:bg-slate-800">Schedule</button>
                    </form>
                    <form action={completeFollowUp.bind(null, lead.id)}><button className="w-full rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold hover:bg-emerald-500">Complete Follow-Up</button></form>
                    <form action={archiveLead.bind(null, lead.id)}><button className="w-full rounded-lg border border-red-500/40 px-4 py-2 text-sm font-semibold text-red-300 hover:bg-red-500/10">Archive</button></form>
                  </div>
                </div>
              </article>
            );
